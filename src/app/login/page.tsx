  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    if (isSignUp) {
      ...
      if (error) {
        ...
        setMessage('Zkontrolujte váš email pro potvrzení registrace.')
      }
    } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center p-4">
      <Card className="w-full max-w-md shadow-xl">
        <CardHeader className="text-center pb-2">
          ...
          <CardTitle className="text-2xl font-bold">PawLog</CardTitle>
          <CardDescription>Digitální zdravotní knížka pro vašeho mazlíčka</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          ...
          <form onSubmit={handleEmailAuth} className="space-y-3">
            ...
            {message && (
              <p className={`text-sm ${message.includes('email') && isSignUp ? 'text-green-600' : 'text-red-500'}`}>
                {message}
              </p>
            )}
